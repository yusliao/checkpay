...
- **2026-05-06** - **DI 手写美分纠偏**：在无 **NN/100** 分列行时，合并上下文中 **首张 `<c>$…</c>$` 上方、与 PAY/ORDER/TO THE 同带的「裸 1～2 位」行**（Vision 常把 **`<c>54/100</c>`** 折断为单独 **`<c>54</c>`**）作为分列分；**`RefineLegalCentsAgainstVisionSpilledLine`** 在 DI **÷100** 得 **50¢** 等与裸分不一致时，以该分为准；单测 `BareSpilled54`
