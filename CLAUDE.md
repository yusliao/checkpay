...
- **2026-05-04** - 公司名称 Vision 解析：`CheckOcrParsingProfile.companyNamePriorRegion`（默认约 `normY 0~0.55`、含支票号下方一行商号，避免旧 `minNormY=0.20` 漏掉 **INC.** 抬头；模板 JSON 可覆盖）；`ParseCompanyName` 对 **INC./LLC/CORP/LTD** 等加权、对 **bank / credit union** 且无高档法人后缀的候选 **0 分**；`ScoreAccountHolderCandidate` 同步剔除付款行印刷名，避免 `company_name` 经 `merged_holder_payto` 回落成银行名；`AzureOcrService` 写 `CompanyName` 与 `Diagnostics.company_name_source`；单测 `ParseCompanyName_IncludesTopPrintedIncAboveOldMinNormY`、`ParseAccountHolderName_RejectsBankBrandingPreferringPayeeLine` 等；排查 [docs/支票OCR失败排查.md](docs/支票OCR失败排查.md)
