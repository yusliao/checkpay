...
- **2026-05-07** - **支票 OCR 成功率默认策略**：`appsettings` / Compose 默认 **`Ocr:PrebuiltCheck:EnrichPrimaryResult=true`**、**`Ocr:ImagePreprocess:Enabled=true`**、**`AmountValidation:TriggerConfidenceThreshold=0.80`**；金额 ROI 支持 **`Mode`**=`OnDemand`|`Always`、**`PrebuiltCheckOnCrop`**（裁剪图再调 `prebuilt-check.us`，`di_roi_crop_*` 与多源共识）；运维说明 [docs/支票OCR黄金回归集.md](docs/支票OCR黄金回归集.md)；根目录 **`README.md`** 技术栈与 Docker env 段落已与上述默认值及 **`OCR_AMOUNT_ROI_*`** 对齐
- **2026-05-07** - **支票金额：ROI 二次 Read + 多源仲裁**：首轮 Vision Read 后按行框估算 **票面数字/大写** 带状 ROI（排除 MICR），**裁剪后再跑一次 Read**（`Ocr:AmountRoiSecondPass:*`，默认开，额外一次 Read）；**仅当**全文大写、`604805.05→6048.05` 类 **嵌入分修复与 ROI 对齐**、或置信/倍数启发允许时替换首轮金额。DI 融合与书面 augmentation 之后：经旁证确认的 **嵌入分修复**；若 **merged / DI Number / DI Word / ROI 裁剪 DI / 全文大写** 中 **≥2 路差额 ≤0.02** 且与 merged 不同则采纳共识。`Diagnostics`：`amount_roi_second_pass`、`amount_roi_second_pass_mode`、`amount_roi_crop_prebuilt`、`amount_embedded_cents_repair`、`amount_multisource_arbitration`、`di_roi_crop_*`；Compose / `.env.example` `OCR_*`；单测 `CheckOcrAmountArbitrationTests`；说明 [docs/支票OCR失败排查.md](docs/支票OCR失败排查.md)
- **2026-05-07** - **`/reports/ach-us`**：表格与 CSV 导出对客户导航 **`Customer`** 使用空条件（`customers` 已软删时 `Include` 后导航可能为 **null**，原 `context.Customer.CustomerCode` 会击毁 Blazor 电路）
...
- **2026-05-07** - **Chase MICR 支票号（无印刷候选）**：`⑈001128⑈ ⑆267084131⑆` 换行 `710978795⑈` 时若 E13B 主路径未写入 `micrBracketedCheck`、末段 ⑈ 启发式误取 **9 位账号**，原仅在与 **PrintedCheckPriorRegion** 内票号对齐时纠偏，Vision 几何漏掉版头 **1128** 仍会落账号；[`ParseCheckNumber`](src/CheckPay.Infrastructure/Services/CheckOcrVisionReadParser.cs) / [`ParseCheckNumberFullText`](src/CheckPay.Infrastructure/Services/CheckOcrVisionReadParser.cs) 在 **`LooksLikeChaseBracketedCheckWithNineDigitAccountTail`** 且已抽出路由前左 bracket 时 **无印刷亦退回 bracket 短形**（如 `1128`）；单测 `ParseCheckNumber_ChaseStyle_WithoutPrintedHintLine_StillPrefersBracketOverAccountTail`
- **2026-05-07** - **旧版全表支票唯一索引进位判重**：若 `ix_check_records_check_number` **无** `WHERE deleted_at IS NULL`，软删行仍占键位会导致「仅查未删为空却保存撞库」；[`CheckNumberGuard.ExistsAsync`](src/CheckPay.Web/Common/CheckNumberGuard.cs) 在 **Npgsql** 下探测 **`pg_index.indpred`**，非部分索引时 **COUNT** 含 **deleted_at IS NOT NULL** 的同键行，与插入语义对齐；排查见 [docs/支票OCR失败排查.md](docs/支票OCR失败排查.md) §0.1；[`CheckUpload`](src/CheckPay.Web/Pages/CheckUpload.razor) 唯一冲突时 **`PostgresException.Detail`** 记 `Warning` 日志
- **2026-05-07** - **重复提示可读性**：[`CheckNumberGuard.FormatDuplicateUserMessage`](src/CheckPay.Web/Common/CheckNumberGuard.cs) 在「同银行支票号已存在」文案中附带 **判重键 ABA**（**空**= 未填路由，与草稿判重键 `coalesce(btrim(routing_number), '')` 一致）；上传/复核/已提交编辑复用；单测 `FormatDuplicateUserMessage_*`
- **2026-05-07** - **判重查询与唯一索引对齐**：[`CheckNumberGuard.ExistsAsync`](src/CheckPay.Web/Common/CheckNumberGuard.cs) 在 **Npgsql** 下用 **`upper(btrim(check_number))` + `coalesce(btrim(routing_number), '')` + `deleted_at IS NULL`** 的 `EXISTS` 与 **`ix_check_records_check_number`** 一致，减少 LINQ 翻译与库侧 upper/trim 语义差异；[`CheckUpload`](src/CheckPay.Web/Pages/CheckUpload.razor) / [`CheckReview`](src/CheckPay.Web/Pages/CheckReview.razor) / [`CheckRecordSubmittedEditDialog`](src/CheckPay.Web/Shared/CheckRecordSubmittedEditDialog.razor) 在捕获 **唯一索引** `DbUpdateException` 后 **Detach** 仍处于 `Added` 的 `CheckRecord`，避免同一会话重试时堆积未提交实体
- **2026-05-07** - **重复支票校验与 DB 唯一索引**：[`CheckNumberGuard.ExistsAsync`](src/CheckPay.Web/Common/CheckNumberGuard.cs) 去掉 `IgnoreQueryFilters`，仅对 **未软删除** 的 `check_records` 判重（与收款列表一致）；迁移 `CheckRecordsUniqueExcludeSoftDeleted` 将 `ix_check_records_check_number` 改为 **`WHERE deleted_at IS NULL`** 的部分唯一索引（与 `customers` 类似），避免「已软删行」阻塞同键再入库；单测 `ExistsAsync_ExcludesSoftDeletedRecords_ReturnsFalse`
- **2026-05-07** - **Chase 类 MICR 支票号**：[`CheckOcrVisionReadParser`](src/CheckPay.Infrastructure/Services/CheckOcrVisionReadParser.cs) 在 `⑈000xxx⑈`、**7** 位 **`001…`** 0 垫片之外，对 **`001128`/`001078`** 等 **6** 位左段（**第 4** 位 **`&lt;4`**）与换行 **9** 位账号作 **左=支票序、右=账号**（与 **`001408`+9** 的 SkipZip 末段票号区分）；无法单靠 MICR 定序时若 **印刷号与路由前左 bracket 去前导 0 等价**则优先较短展示；单测 `ParseCheckNumber_ChaseStyle_SixOrSevenDigitZeroPadBracket_NotNineDigitAccount`
- **2026-05-07** - **训练样本强匹配误伤**：[`CheckOcrParsedSampleCorrector`](src/CheckPay.Infrastructure/Services/CheckOcrParsedSampleCorrector.cs) 的 `IsStrongMatch` 除「样本 OCR 支票号 = 当前解析支票号 + 路由」外，增加 **MICR 行数字指纹**（及双方的 `CheckNumberMicr`）一致校验；避免同一路由下多张票因 **同一误解析支票号**（如均将磁墨账号误作票号）而套用另一张票的纠偏；单测 `ApplyIfMatched_ShouldNotStrongMatch_WhenSameCheckNumberRouting_ButMicrDigitFingerprintDiffers`
...
- **2026-05-06** - 支票 **金额（百分比分列 非写死）**：`TryParseAmountFromWords` 可选 **`fullTextForStandaloneFraction`**，在大写行内无 **`xx/100`** 时从全文拾取 **独立行 `XX/100`/`%`/`percent`**（多行候选时取**首个在含 `$` 数字行之后**者，跳过磁墨行）；**DI `prebuilt-check.us`**：`PrebuiltCheckStructuredExtractor` 读 **`WordAmount`/`AmountInWords`/`LegalAmount`**，用同一解析 + **`AnalyzeResult.Content`** 补分列；**`MergePrebuiltStructuredFields`** 在整数美元与 Vision 一致且 Vision 小数≈0 时采纳 **`WordAmountParsed`**；`ExtractLegalAmountFromDiV4` / 手写校验路径传全文；Vision 路径仍用 **`ApplyWrittenAmountAugmentationFromVisionText`**（`amount_written_augmentation`）；单测 `AzureOcrAmountParserTests`
- **2026-05-06** - **MICR 三行版式 + `micr_line_raw` 链式合并**：`ExpandMicrInkLinesByVerticalProximity` 按 **NormTop/Bottom 扩边条带重叠**做 **传递闭包**（不再只与含路由种子比 centerY≤0.14）；**纯数字短行**与已选磁墨行 **touch≈0.05**；**Citizens 类**：`⑆ABA⑆` 独行 + `账号…⑈` + 下行纯数字支票号 → `TryExtractMicrSplitTransitRowAccountThenCheckDigitRow`，避免末段 `(\d+)⑈` 把 **账号当支票号**；单测 `TryResolveMicrLineRawFromLayout_TransitiveMergeLinksRoutingRowToDistantOnUsAndDigitLine`、`ParseCheckNumber_CitizensStyle_SplitMicrThreeRows_AccountLineNotTakenAsCheck`
- **2026-05-06** - **MICR 分行 + 漏读左 ⑈**：`TryClassifyOnUsDigitsAroundRouting` 支持 `⑆路由⑆` 后 **`nn…⑈`**（无左侧 ⑈）作账号；**`000xxx`（6～7 位）+ 下行 9 位** 时定为 **Chase 类**（支票序≠账号）；`TryPickBracketedMicrCheckShortPairFallback` 换行取右时排除此类；`TryResolveMicrLineRawFromLayout` 并入 **`\d{6,17}⑈`** 行；**未**实现透视四角拉平（`image_preprocess_perspective_correction=skipped`）；单测 `ParseCheckNumber_ChaseStyle_*`
- **2026-05-06** - 支票 **Vision 金额 `/100` 分列**：票面小写支持 **`$ 1234 56/100`**、**`56/100`** 独立行拼回、全文换行、可选 **`%`/`percent`**；**空格两位分**正则不吞 **`56/100`**（负向先行）；`TryParseAmountFromWords` 支持全角 **`／`** **`％`** 与 **`48/100%`**；`amount_parse_mode` **`fraction_100`** / **`spillover_fraction_100`** / **`newline_fraction_100`** 纳入 Worker 仍跑手写校验；单测 `ParseAmount_FractionOver100_*`、`TryParseAmountFromWords(…56/100…)`
- **2026-05-06** - 支票 **Vision 金额（续）**：**相邻行拆字** `<c>$ 10148</c>` + 下行 `<c>48</c>` 几何可接时走 **`spillover_cents`**；**全文** `<c>\n</c>` 间两位分走 **`newline_cents`**；`Diagnostics.amount_parse_mode`；**Worker** 对 **`space_cents_inline` / `spillover_cents` / `newline_cents`** 即使金额置信度 ≥ 阈值仍 **跑一次 DI 手写金额校验** 以补齐大写解析；`TryParseAmountFromWords` 增 **`yourty→forty`、`fourty`、`por`** OCR 纠偏；单测 `ParseAmount_SpilloverCentsOnAdjacentLine_*`、`TryParseAmountFromWords(…yourty…)`
...
- **2026-05-06** - **支票图像预处理**（可选）：`Ocr:ImagePreprocess` **Vision Read / DI 前**对 MinIO 原图做 **纠倾**、**亮纸轴对齐裁剪**、**长边放大**（SixLabors.ImageSharp）；`Diagnostics` 增加 `image_preprocess_*` 与 `image_preprocess_skew_applied_bucket`；默认 **`Enabled: false`**；Compose / `.env.example` `OCR_IMAGE_PREPROCESS_ENABLED`；文档 [docs/支票OCR离线评估与预处理.md](docs/支票OCR离线评估与预处理.md)；单测 `CheckImagePreprocessorTests`
- **2026-05-06** - 支票 **Vision 解析**（Peoples 等）：MICR **`⑈≤7位⑈ ⑆ABA⑆`** 与 **≥8 位账号** 换行相接且 `max&lt;10` 不分类时，**短段为支票号**（修复误取账号如 `00326291`）；**`ParseDate`** 对 **独立 `DATE` 邻行**加权、对 **门牌 + `MM-DD-YY` + 州邮编** 夹心 hyphen 日降权；**`AzureOcrService`** 剔除与 **Pay to** 重复的 Vision **`bank_name`**；文档 [docs/支票OCR失败排查.md](docs/支票OCR失败排查.md)；单测 `ParseCheckNumber_PeoplesStyle_*`、`ParseDate_PeoplesStyle_*`
...
- **2026-05-05** - 支票 **金额 / 印刷号 / 公司名**：`AmountDollarHyphenCentsRegex` 识别 **`$ 686-25` → 686.25**；`ParseAmount` 对 hyphen 金额加权、**含 `$\`** 的左上带加分，**顶部极浅行且无 `$\*` 的裸 `x.xx` 小数**降权（抗 `4126.26` 类噪声）；`ParseAmountFullText` 亦优先 hyphen；`PickBestPrintedCheckCandidate` **跳过磁墨行与 FOR INV/FOR I 备忘行**；印刷号与磁墨对齐时 **`fragMicrCheckDigits` 可作 hint**（`micrNumber` 为空时仍可对齐 `535`↔`0535`）；`GetCorporateLegalSuffixBump` 增 **`Lic`**（OCR 类 LLC）；`ScoreCompanyNameCandidate` **重罚行内 `$\``+hyphen/dot 金额**（付款目录 + 金额行）、**normY≤0.12** 抬头微加分；单测 `ParseAmount_PrefersDollarHyphen686OverTopNoiseDecimal`、`ParseCompanyName_PrefersTopLicPrintedNameOverPayeeCatalogAmountLine`、`ParseCheckNumber_RejectsForMemoSevenDigit_OverPrinted535_WithMicr0535Hint`
- **2026-05-05** - 支票 **碎片 MICR / 支票号**：`ParseCheckNumber` 在 **transit 碎片账号合并命中且无磁墨支票序号行**（纯数字 `0535` 未进入所用 micr 切片亦如此）时 **跳过**末段 `(\d+)⑈` 启发式（避免把账号 mid `7191⑈` 当支票号）；印刷 **535** 仍可胜出；手写金额：`TryParseAmountFromWords` 支持尾部 **`nn!`** 分值、`Eightysix` 等 OCR 粘连英文数字词；DI `ExtractLegalAmountFromDiV4` 在结构化字段不可用时的 **`AnalyzeResult.Content` 逐行兜底**（须含 hundred/thousand/million）；主路径 DI 融合采纳 **`MICR.CheckNumber` 长度 ≥3**；单测 `ParseCheckNumber_TransitFragmentMerge_Printed535WhenPureMicrCheckRowOutsideMicrSlice`、`AzureOcrAmountParserTests.Six_Hundred_Eightysix`
- **2026-05-05** - MICR **路由块碎片**：`TryRecoverTransitFragmentMicrSplit` 合并 `⑆ABA⑆` 行内紧随的账号前缀数字、下行折叠后的 `7191⑈` 形态与**无 ⑆/⑈**的纯数字支票行（如 `0535`），恢复长账号（如 `16307191`）；已知 9 位路由时 `TryResolveMicrLineRawFromLayout` 并入相邻磁墨带 **3～8 位纯数字行**并按纵坐标排序输出；`ParseCheckNumber` 吸收碎片磁墨支票序并与 **3 位**印刷候选对齐（`535`）；`micr_selection_mode`：`e13b_transit_fragment_merge`；单测 `ParseMicrHeuristic_TransitFragmentMerge_CommercialBankStyleSplitMicr`、`TryResolveMicrLineRawFromLayout_IncludesAdjacentDigitOnlyMicrBandLine`、`ParseCheckNumber_TransitFragmentMerge_AlignsPrintedThreeDigit535`
- **2026-05-05** - 支票 **`check_number`**：`TryExtractMicrCheckBracketedAroundTransit` 在 E13B `⑈…⑈ ⑆路由⑆ …⑈` 匹配成功但 `TryAssignMicrCheckVsAccountByLength` 因 max&lt;10 放弃时，增加 **支票号专用** 后备（max≤7→较短段，max≥8→较长段），避免两行磁墨时误取末段 `2037489⑈` 而漏 `003122`；**不**改变 `ParseMicrHeuristic` 账号分类；单测 `ParseCheckNumber_MetroCityBankSplitMicr_*`
