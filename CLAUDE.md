...
- **2026-05-06** - 支票 **金额（分列 percent + Vision `00`）**：`TryParseAmountFromWords` 对 **`$ … 00` + `Ten thousand one hundred and yourty eight por DOLLARS`** 类粘连补 **`forty-eight and 54/100`**；`ProcessCheckImageAsync` 在 prebuilt 融合后 **`ApplyWrittenAmountAugmentationFromVisionText`**：Vision 整数分与小写非零分且**整数美元一致**时采纳全文英文行（`amount_written_augmentation`）；单测 `AzureOcrAmountParserTests`
