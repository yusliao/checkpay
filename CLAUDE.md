...
- **2026-05-04** - 公司名称 Vision 解析：`CheckOcrParsingProfile.companyNamePriorRegion`（默认约 `normY 0~0.62` / `maxNormX≈0.99`；主带无命中时 **上半张回退** 再排分）；`ParseCompanyName` 对 **INC./LLC…** 加权、**bank/credit union** 且无高档法人后缀 **0 分**；`ScoreAccountHolderCandidate` 剔付款行；**prebuilt-check.us** 融合时 `PayerName` 若与 **BankName** 相同或属付款行字样则 **不覆盖** Vision 持有人，避免 `merged_holder_payto` 把 `company_name` 写成银行名；`AzureOcrService` 与 `Diagnostics.company_name_source`；单测含 `ParseCompanyName_FallbackFindsIncLineWhenOutsideTemplateCompanyBand`、`ShouldSkipDiPayerNameForAccountHolder_*`；排查 [docs/支票OCR失败排查.md](docs/支票OCR失败排查.md)
