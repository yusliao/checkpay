...
- **2026-05-06** - **UI 文案**：支票上传/复核、客户管理列表与新增编辑对话框、客户 CSV 模板/导出表头中，原「手机号」用户可见名称统一为 **餐馆编号**（库字段仍 `customers.mobile_phone` / `MobilePhone`）；导入仍接受旧表头 **手机号**。**ACH 支票导出**（`/reports/ach-us`）与 **ACH 已扣款导出**（`/reports/ach-cn`）页面列与 **导出 CSV 表头** 中对应列同步为 **餐馆编号**。**收款记录**（`/records`、`CheckRecordsAdmin`）列表/抽屉与 **`CheckRecordSubmittedEditDialog`** 票面编辑中的「手机号」文案同步为 **餐馆编号**。
